...
final class AppModel: ObservableObject {
  ...
  func credentialHealth(_ account: AccountRecord) -> CredentialHealth {
    guard let credential = try? keychain.credential(accountID: account.id) else {
      return CredentialHealth(title: "需重登", icon: "exclamationmark.triangle.fill", color: .red)
    }
    if account.provider == .deepseek || account.provider == .minimax || account.provider == .glm || account.provider == .copilot {
      return CredentialHealth(title: "已保存", icon: "checkmark.shield.fill", color: .green)
    }
    if let expires = credential.expiresAt {
      if expires <= .now {
        if credential.refreshToken?.isEmpty == false {
          return CredentialHealth(title: "可续期", icon: "arrow.triangle.2.circlepath", color: .orange)
        }
        return CredentialHealth(title: "需重登", icon: "exclamationmark.triangle.fill", color: .red)
      }
      if expires.timeIntervalSinceNow < 3600 {
        return CredentialHealth(title: "即将续期", icon: "clock.arrow.circlepath", color: .orange)
      }
    }
    if snapshots[account.id]?.stale == true {
      return CredentialHealth(title: "缓存中", icon: "icloud.slash", color: .orange)
    }
  ...
  func refresh(_ account: AccountRecord) async {
    do {
      ...
      if let snapshot = await SharedStore.shared.snapshot(for: account.id) { snapshots[account.id] = snapshot }
    }
  }

  func refreshAll() async {
    isBusy = true
    let refreshed = await UsageService.shared.refresh(accountIDs: accounts.filter(\.isEnabled).map(\.id))
  ...
  func delete(_ account: AccountRecord) async {
    do { try keychain.deleteCredential(accountID: account.id) } catch { errorMessage = error.localizedDescription }
  ...
  func setEnabled(_ account: AccountRecord, enabled: Bool) async {
    var updated = account; updated.isEnabled = enabled
  ...
  func rename(_ account: AccountRecord, label: String) async {
    var updated = account; updated.label = label.trimmingCharacters(in: .whitespacesAndNewlines)
  ...
  func move(_ account: AccountRecord, offset: Int) async {
    ...
    let to = min(max(0, from + offset), accounts.count - 1); guard to != from else { return }
    var reordered = accounts; let item = reordered.remove(at: from); reordered.insert(item, at: to)
    accounts = reordered.enumerated().map { index, record in var updated = record; updated.sortOrder = index; return updated }
    await SharedStore.shared.saveAccounts(accounts)
    WidgetCenter.shared.reloadAllTimelines()
  }

  private func normalizeAndSaveOrder() async {
    accounts = accounts.enumerated().map { index, record in var updated = record; updated.sortOrder = index; return updated }
    await SharedStore.shared.saveAccounts(accounts)
  }
  ...
  private func saveCredential(_ credential: Credential, provider: ProviderID) async throws {
    ...
    if let providerID = identity.providerAccountID,
      let existing = accounts.first(where: { $0.provider == provider && $0.providerAccountID == providerID }) {
    ...
    if !provider.supportsMultipleAccounts, let existing = accounts.first(where: { $0.provider == provider }) {
      try keychain.saveCredential(credential, accountID: existing.id)
      _ = try? await UsageService.shared.refresh(accountID: existing.id)
      await load(); WidgetCenter.shared.reloadAllTimelines(); return
    }
    ...
    if let snapshot = try? await UsageService.shared.refresh(accountID: record.id) { snapshots[record.id] = snapshot }
    WidgetCenter.shared.reloadAllTimelines()
  }

  private func identityForCredential(provider: ProviderID, credential: Credential) -> (email: String?, providerAccountID: String?, shortAccountLabel: String?) {
    let claims = decodeJWTPayload(credential.idToken ?? credential.accessToken) ?? [:]
    let email = findNestedString(claims, key: "email")
    let accountID = credential.accountID ?? findNestedString(claims, key: "chatgpt_account_id") ?? findNestedString(claims, key: "sub")
  ...
  private func performBusy(_ operation: () async throws -> Void) async {
    ...
    do { try await operation(); await load() } catch { errorMessage = error.localizedDescription }
  }
}
