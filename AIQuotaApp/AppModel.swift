...
final class AppModel: ObservableObject {
  ...
  func importConfig(_ data:Data,replace:Bool) async { isBusy=true;defer{isBusy=false};do{let result=try await PortableConfigImporter.shared.importData(data,mode:replace ? .replace:.merge);await load();statusMessage="导入完成：新增 \(result.added)，更新 \(result.updated)，凭据 \(result.credentialsImported)";await refreshAll()}catch{errorMessage=error.localizedDescription} }
  ...
  func addAPIKey(provider:ProviderID,key:String,baseURL:String?=nil)async{await performBusy{try await saveCredential(Credential(accessToken:key,baseURL:baseURL?.trimmingCharacters(in:.whitespacesAndNewlines)),provider:provider)}}
  ...
  func delete(_ account:AccountRecord)async{try? keychain.deleteCredential(accountID:account.id);await SharedStore.shared.removeAccount(account.id);accounts.removeAll{$0.id==account.id};snapshots[account.id]=nil;cooldowns[account.id]=nil;await normalizeAndSaveOrder();WidgetCenter.shared.reloadAllTimelines()}
