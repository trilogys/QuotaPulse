...
struct ContentView: View {
  ...
  var body: some View {
    NavigationStack {
      List {
        Section {
          if model.accounts.isEmpty { ContentUnavailableView("还没有账号",systemImage:"person.crop.circle.badge.plus",description:Text("右上角 + 添加 Codex、Claude、Kimi 或 API 平台。")) }
        ...
        Section("小组件"){Label("小组件中的 ↻ 会原地刷新，不打开 App",systemImage:"arrow.clockwise.circle");Label("普通点小组件区域才会进入这里",systemImage:"hand.tap")}.font(.footnote).foregroundStyle(.secondary)
      }
      .navigationTitle("AI Quota")
      .toolbar{ToolbarItem(placement:.topBarLeading){Button{showingSettings=true}label:{Image(systemName:"gearshape")}};ToolbarItem(placement:.topBarTrailing){Menu{Button("Codex · ChatGPT 登录"){loginOAuth(.codex)};Button("Claude · OAuth 登录"){loginOAuth(.claude)};Button("Kimi · OAuth 登录"){loginOAuth(.kimi)};Divider();Button("DeepSeek · API Key"){apiProvider=.deepseek};Button("MiniMax · Coding Key"){apiProvider=.minimax};Button("GLM · Coding Key"){apiProvider=.glm};Button("GitHub Copilot · Token"){apiProvider=.copilot}}label:{Image(systemName:"plus")}}}
...
struct SettingsView:View{
  @ObservedObject var model:AppModel;@Environment(\.dismiss) private var dismiss;@State private var autoMinutes=15
  var body:some View{NavigationStack{Form{Section("自动刷新"){Picker("最早刷新间隔",selection:$autoMinutes){Text("10 分钟").tag(10);Text("15 分钟").tag(15);Text("30 分钟").tag(30);Text("1 小时").tag(60);Text("2 小时").tag(120)};Text("这是 WidgetKit 的最早刷新时间，实际自动刷新由 iOS 调度；小组件里的 ↻ 属于用户主动刷新，会立即执行 App Intent。").font(.footnote).foregroundStyle(.secondary)};Section("数据"){NavigationLink{BackupSettingsView(model:model)}label:{Label("导入与导出",systemImage:"arrow.up.arrow.down.square")};Text("配置文件可在 iOS 与 Android 之间共用。完整备份可包含登录凭据。 ").font(.footnote).foregroundStyle(.secondary)};Section("凭据状态"){ForEach(model.accounts){account in let health=model.credentialHealth(account);HStack{VStack(alignment:.leading){Text(account.label);Text(account.provider.title).font(.caption).foregroundStyle(.secondary)};Spacer();Label(health.title,systemImage:health.icon).foregroundStyle(health.color).font(.caption)}}};Section("显示账号"){ForEach(model.accounts){account in Toggle(account.label,isOn:Binding(get:{account.isEnabled},set:{value in Task{await model.setEnabled(account,enabled:value)}}))}}}.navigationTitle("设置").toolbar{ToolbarItem(placement:.confirmationAction){Button("完成"){dismiss()}}}.task{autoMinutes=await SharedStore.shared.autoRefreshMinutes()}.onChange(of:autoMinutes){_,newValue in Task{await SharedStore.shared.setAutoRefreshMinutes(newValue);WidgetCenter.shared.reloadAllTimelines()}}}}
}
