...
struct AIQuotaProvider: TimelineProvider {
  func placeholder(in context: Context) -> AIQuotaEntry {
    AIQuotaEntry(
      ...
      items: [
        WidgetDisplayItem(
          account: AccountRecord(provider: .codex, label: "Codex · Work"),
          snapshot: UsageSnapshot(
            accountID: UUID(),
            provider: .codex,
            windows: [
              UsageWindow(id: "5h", label: "5h", remainingPercent: 72),
              UsageWindow(id: "week", label: "周", remainingPercent: 48),
            ]
          )
        )
      ],
  ...
  func getSnapshot(in context: Context, completion: @escaping (AIQuotaEntry) -> Void) {
    Task {
      completion(await makeEntry(family: context.family))
    }
  }
  ...
  private func makeEntry(family: WidgetFamily) async -> AIQuotaEntry {
    ...
    for account in accounts {
      let snapshot = await SharedStore.shared.snapshot(for: account.id)
      items.append(WidgetDisplayItem(account: account, snapshot: snapshot))
    }
  ...
  private func snapshotBody(_ snapshot: UsageSnapshot) -> some View {
    if let balance = snapshot.balance {
      ...
      HStack(spacing: family == .systemSmall ? 5 : 8) {
        ForEach(Array(snapshot.windows.prefix(family == .systemSmall ? 2 : 3))) { window in
          quotaPill(window)
        }
      }
    } else if !snapshot.metrics.isEmpty {
      HStack(spacing: 8) {
        ForEach(Array(snapshot.metrics.prefix(2))) { metric in
