...
struct AIQuotaProvider: AppIntentTimelineProvider {
      ...
      items: [
        WidgetDisplayItem(
          ...
          snapshot: UsageSnapshot(
            ...
            provider: .codex,
            windows: [
              UsageWindow(id: "5h", label: "5h", remainingPercent: 72, resetAt: .now.addingTimeInterval(7200)),
              UsageWindow(id: "week", label: "周", remainingPercent: 48, resetAt: .now.addingTimeInterval(172800)),
            ]
          )
        )
      ],
  ...
  func snapshot(
    for configuration: AIQuotaWidgetConfigurationIntent,
    in context: Context
  ) async -> AIQuotaEntry {
    await makeEntry(configuration: configuration, family: context.family)
  }

  func timeline(
    for configuration: AIQuotaWidgetConfigurationIntent,
    in context: Context
  ) async -> Timeline<AIQuotaEntry> {
    ...
    return Timeline(
      entries: [entry],
      policy: .after(Date().addingTimeInterval(TimeInterval(minutes * 60)))
    )
  }

  private func makeEntry(
    configuration: AIQuotaWidgetConfigurationIntent,
    family: WidgetFamily,
    ...
    for account in accounts {
      items.append(
        WidgetDisplayItem(
          account: account,
          snapshot: await SharedStore.shared.snapshot(for: account.id)
        )
      )
    }
    return AIQuotaEntry(
  ...
  private func configuredAccounts(
    ...
    case .account:
      guard let rawID = configuration.account?.id,
    ...
    HStack(spacing: 8) {
      VStack(alignment: .leading, spacing: 4) {
        ...
        if let snapshot = item.snapshot {
          snapshotBody(snapshot).invalidatableContent(true)
        } else {
          Text("等待首次刷新")
            .font(.system(size: 9))
            .foregroundStyle(.secondary)
...
struct AIQuotaWidget: Widget {
  ...
  var body: some WidgetConfiguration {
    ...
    .description("Codex、Claude、Kimi 等 AI 额度。支持单 Provider、单账号和小组件内刷新。")
