...
struct AIQuotaProvider: AppIntentTimelineProvider {
  ...
  private func makeEntry(configuration: AIQuotaWidgetConfigurationIntent, family: WidgetFamily, accounts suppliedAccounts: [AccountRecord]? = nil) async -> AIQuotaEntry {
    let accounts = suppliedAccounts ?? configuredAccountsSyncPlaceholder()
  ...
  private func configuredAccounts(_ configuration: AIQuotaWidgetConfigurationIntent, family: WidgetFamily) async -> [AccountRecord] {
    let enabled = await SharedStore.shared.accounts().filter(\.isEnabled)
    let filtered: [AccountRecord]
    switch configuration.mode {
    case .all: filtered = enabled
    case .provider: filtered = enabled.filter { $0.provider == configuration.provider.providerID }
    case .account:
      guard let rawID = configuration.account?.id, let id = UUID(uuidString: rawID), let account = enabled.first(where: { $0.id == id }) else { return [] }
      filtered = [account]
    }
