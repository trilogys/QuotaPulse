...
actor SharedStore {
  ...
  private enum Key {
    ...
    static let backgroundRefreshEnabled = "backgroundRefreshEnabled.v1"
    static let lastSuccessfulRefreshAt = "lastSuccessfulRefreshAt.v1"
  ...
  func autoRefreshMinutes()->Int{let value=defaults.integer(forKey:Key.autoRefreshMinutes);return (10...1_440).contains(value) ? value : 15}
  func setAutoRefreshMinutes(_ minutes:Int){defaults.set(min(1_440,max(10,minutes)),forKey:Key.autoRefreshMinutes)}
  ...
  func backgroundRefreshEnabled()->Bool{defaults.object(forKey:Key.backgroundRefreshEnabled)==nil ? true:defaults.bool(forKey:Key.backgroundRefreshEnabled)}
  func setBackgroundRefreshEnabled(_ enabled:Bool){defaults.set(enabled,forKey:Key.backgroundRefreshEnabled)}
  func lastSuccessfulRefreshAt()->Date?{let value=defaults.double(forKey:Key.lastSuccessfulRefreshAt);return value>0 ? Date(timeIntervalSince1970:value):nil}
