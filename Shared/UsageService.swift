...
actor UsageService {
    ...
    -> UsageSnapshot
  {
    ...
    for (key, rawAny) in rateLimit {
      guard key.hasSuffix("_window"), let raw = dictionary(rawAny), number(raw["used_percent"]) != nil
      else { continue }
      let duration = int64(raw["limit_window_seconds"]) ?? 0
      let used = number(raw["used_percent"]) ?? 0
      let resetAt: Date?
      if let epoch = int64(raw["reset_at"]), epoch > 0 {
        resetAt = Date(timeIntervalSince1970: TimeInterval(epoch))
      } else if let seconds = int64(raw["reset_after_seconds"]), seconds >= 0 {
    ...
    -> Credential
  {
    ...
    let body = formBody([
      ...
      "client_id": credential.clientID ?? "app_EMoamEEZ73f0CkXaXp7hrann",
    ])
    ...
    guard (200..<300).contains(result.statusCode) else {
      throw UsageError.refreshFailed("Codex HTTP \(result.statusCode)")
    }
    let json = try result.jsonDictionary()
    guard let access = string(json["access_token"]) else {
      throw UsageError.refreshFailed("Codex missing access_token")
    }
    var updated = credential
    updated.accessToken = access
    updated.refreshToken = string(json["refresh_token"]) ?? refreshToken
    updated.idToken = string(json["id_token"]) ?? credential.idToken
  ...
  private func fetchClaude(account: AccountRecord, credential: inout Credential) async throws
    -> UsageSnapshot
  {
    if let expires = credential.expiresAt, expires.timeIntervalSinceNow < 60,
    ...
    let root = try result.jsonDictionary()
    var windows: [UsageWindow] = []
    if let five = dictionary(root["five_hour"]) {
      windows.append(
        UsageWindow(
          id: "claude-5h", label: "5h",
          remainingPercent: 100 - (number(five["utilization"]) ?? 0),
          resetAt: parseISODate(string(five["resets_at"]))
        ))
    }
    if let seven = dictionary(root["seven_day"]) {
      windows.append(
        UsageWindow(
          id: "claude-week", label: "周",
          remainingPercent: 100 - (number(seven["utilization"]) ?? 0),
          resetAt: parseISODate(string(seven["resets_at"]))
        ))
    }
    ...
    -> Credential
  {
    ...
    let payload: [String: Any] = [
      "grant_type": "refresh_token",
      "refresh_token": refreshToken,
      "client_id": credential.clientID ?? "9d1c250a-e61b-44d9-88ed-5944d1962f5e",
    ]
    ...
    guard (200..<300).contains(result.statusCode) else {
      throw UsageError.refreshFailed("Claude HTTP \(result.statusCode)")
    }
    let json = try result.jsonDictionary()
    guard let access = string(json["access_token"]) else {
      throw UsageError.refreshFailed("Claude missing access_token")
    }
    var updated = credential
    updated.accessToken = access
    updated.refreshToken = string(json["refresh_token"]) ?? refreshToken
    if let seconds = number(json["expires_in"]) {
      updated.expiresAt = Date().addingTimeInterval(seconds)
    }
    try keychain.saveCredential(updated, accountID: accountID)
    return updated
  }
    ...
    -> UsageSnapshot
  {
    ...
    func call(_ credential: Credential) async throws -> HTTPResult {
      var headers = ["Authorization": "Bearer \(credential.accessToken)", "Accept": "application/json"]
      credential.deviceHeaders?.forEach { headers[$0.key] = $0.value }
      return try await http.send(URL(string: "https://api.kimi.com/coding/v1/usages")!, headers: headers)
    }
    var result = try await call(credential)
    ...
    if let usage = dictionary(root["usage"]), !windows.contains(where: { $0.label == "周" }),
    ...
    -> Credential
  {
    guard let refreshToken = credential.refreshToken else { throw UsageError.unauthorized }
    let body = formBody([
      "client_id": credential.clientID ?? "17e5f671-d194-4dfb-9706-5516cb48c098",
      "grant_type": "refresh_token",
      "refresh_token": refreshToken,
    ])
    ...
    guard (200..<300).contains(result.statusCode) else {
      throw UsageError.refreshFailed("Kimi HTTP \(result.statusCode)")
    }
    let json = try result.jsonDictionary()
    guard let access = string(json["access_token"]) else {
      throw UsageError.refreshFailed("Kimi missing access_token")
    }
    var updated = credential
    updated.accessToken = access
    updated.refreshToken = string(json["refresh_token"]) ?? refreshToken
    if let seconds = number(json["expires_in"]) { updated.expiresAt = Date().addingTimeInterval(seconds) }
    try keychain.saveCredential(updated, accountID: accountID)
    return updated
  }
    ...
    -> UsageSnapshot
  {
    ...
    let preferred = infos.first(where: { string($0["currency"]) == "CNY" })
      ?? infos.first(where: { string($0["currency"]) == "USD" }) ?? infos[0]
    ...
    -> UsageSnapshot
  {
    ...
    let endpoint = URL(string: "\(base)/v1/api/openplatform/coding_plan/remains")!
    let result = try await http.send(
      endpoint,
      headers: [
    ...
    let current = number(data["currentUsage"] ?? data["current_usage"] ?? data["used"])
    let total = number(data["totalUsage"] ?? data["total_usage"] ?? data["limit"])
    if let current, let total, total > 0 {
      return UsageSnapshot(
        accountID: account.id, provider: .minimax,
        windows: [
          UsageWindow(
            id: "minimax-plan", label: "计划",
            remainingPercent: ((total - current) / total) * 100)
        ],
        metrics: [
          UsageMetric(label: "已用", value: formatNumber(current)),
          UsageMetric(label: "总量", value: formatNumber(total)),
        ]
      )
    }
    return UsageSnapshot(
      accountID: account.id, provider: .minimax,
      metrics: [UsageMetric(label: "状态", value: "已连接")])
  }

  // MARK: GLM

  private func fetchGLM(account: AccountRecord, credential: Credential) async throws -> UsageSnapshot {
    let base = normalizedBaseURL(credential.baseURL, fallback: "https://open.bigmodel.cn")
    let endpoints = [
      "\(base)/api/monitor/usage/quota/limit",
      "\(base)/api/paas/v4/usage/quota",
    ]
    var lastFailure = ""
    for endpoint in endpoints {
      ...
      guard (200..<300).contains(result.statusCode) else {
        lastFailure = "HTTP \(result.statusCode)"
        continue
      }
      let root = try result.jsonDictionary()
      let data = dictionary(root["data"]) ?? dictionary(root["result"]) ?? root
      if let percentage = number(
        data["remainingPercent"] ?? data["remaining_percent"] ?? data["percentage"])
      {
        return UsageSnapshot(
          accountID: account.id, provider: .glm,
          windows: [
            UsageWindow(id: "glm-plan", label: "计划", remainingPercent: percentage)
          ])
      }
      let metrics = numericMetrics(
        data,
        keys: ["usage", "used", "limit", "quota", "remaining", "balance"])
      if !metrics.isEmpty {
        return UsageSnapshot(accountID: account.id, provider: .glm, metrics: metrics)
      }
    }
    throw UsageError.invalidResponse("GLM usage unavailable \(lastFailure)")
  }
    ...
    -> UsageSnapshot
  {
    ...
    if let quotaSnapshots = dictionary(root["quota_snapshots"]) {
      for (key, raw) in quotaSnapshots {
        guard let item = dictionary(raw) else { continue }
        if let percent = number(item["percent_remaining"]) {
          windows.append(
            UsageWindow(id: "copilot-\(key)", label: displayLabel(key), remainingPercent: percent))
        } else if let remaining = number(item["remaining"]), let entitlement = number(item["entitlement"]),
          entitlement > 0
        {
          windows.append(
            UsageWindow(
              id: "copilot-\(key)", label: displayLabel(key),
              remainingPercent: (remaining / entitlement) * 100))
          metrics.append(
            UsageMetric(
              label: displayLabel(key),
              value: "\(formatNumber(remaining))/\(formatNumber(entitlement))"))
        }
      }
    }
    if windows.isEmpty {
      metrics.append(contentsOf: numericMetrics(root, keys: ["quota", "remaining", "usage", "limit"]))
    }
    return UsageSnapshot(
      accountID: account.id, provider: .copilot, windows: windows, metrics: metrics)
  }

  // MARK: Helpers

  private func uniqueWindows(_ windows: [UsageWindow]) -> [UsageWindow] {
    var labels = Set<String>()
    return windows.filter { labels.insert($0.label).inserted }
  }

  private func durationLabel(seconds: Int64) -> String {
    switch seconds {
    case 1...21_600: return "\(max(1, seconds / 3600))h"
    case 21_601...691_200: return seconds >= 518_400 ? "周" : "\(max(1, seconds / 86_400))d"
    default: return "额度"
    }
  }

  private func kimiDurationSeconds(_ value: [String: Any]) -> Int64 {
    let duration = int64(value["duration"]) ?? 0
    let unit = (string(value["timeUnit"] ?? value["time_unit"]) ?? "")
      .uppercased().replacingOccurrences(of: "TIME_UNIT_", with: "")
    if unit.hasPrefix("SECOND") { return duration }
    if unit.hasPrefix("MINUTE") { return duration * 60 }
    if unit.hasPrefix("HOUR") { return duration * 3600 }
    if unit.hasPrefix("DAY") { return duration * 86_400 }
    return 0
  }

  private func parseKimiDetail(_ value: [String: Any], duration: Int64, id: String) -> UsageWindow? {
    guard let limit = number(value["limit"]), limit > 0 else { return nil }
    let used = number(value["used"]) ?? (limit - (number(value["remaining"]) ?? limit))
    let remaining = ((limit - used) / limit) * 100
    let resetKeys = ["resetAt", "reset_at", "resetTime", "reset_time"]
    var resetAt: Date?
    for key in resetKeys {
      if let parsed = flexibleDate(value[key]) { resetAt = parsed; break }
    }
    if resetAt == nil {
      for key in ["resetIn", "reset_in", "ttl"] {
        if let seconds = int64(value[key]), seconds >= 0 {
          resetAt = Date().addingTimeInterval(TimeInterval(seconds)); break
        }
      }
    }
    return UsageWindow(
      id: id, label: durationLabel(seconds: duration), remainingPercent: remaining, resetAt: resetAt)
  }

  private func numericMetrics(_ value: [String: Any], keys: [String]) -> [UsageMetric] {
    keys.compactMap { key in
      guard let value = number(value[key]) else { return nil }
      return UsageMetric(label: displayLabel(key), value: formatNumber(value))
    }
  }

  private func normalizedBaseURL(_ value: String?, fallback: String) -> String {
    (value?.trimmingCharacters(in: CharacterSet(charactersIn: "/"))).flatMap { $0.isEmpty ? nil : $0 }
      ?? fallback
  }
  ...
  private func formBody(_ values: [String: String]) -> Data {
    ...
    }.sorted().joined(separator: "&").data(using: .utf8) ?? Data()
  }
  ...
  private func flexibleDate(_ value: Any?) -> Date? {
    ...
    if let string = value as? String { return parseISODate(string) }
    return nil
  }
