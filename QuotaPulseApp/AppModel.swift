...
final class AppModel: ObservableObject {
  ...
  func importConfig(_ data:Data,replace:Bool) async { errorMessage=nil;statusMessage=nil;isBusy=true;defer{isBusy=false};do{let result=try await PortableConfigImporter.shared.importData(data,mode:replace ? .replace:.merge);await load();let skipped=result.skippedAccounts>0 ? "，跳过 \(result.skippedAccounts)":"";let proxy=result.proxyImported ? "，代理已导入":"";statusMessage="\(result.source.title) 导入完成：新增 \(result.added)，更新 \(result.updated)，凭据 \(result.credentialsImported)\(skipped)\(proxy)";Task { await self.refreshAll() }}catch{errorMessage=error.localizedDescription} }
  ...
  func reauthenticate(_ account:AccountRecord,presenter:UIViewController)async{await performBusy{let credential:Credential;switch account.provider{case .codex:credential=try await CodexOAuthCoordinator().login(presenting:presenter);case .claude:credential=try await ClaudeOAuthCoordinator().login(presenting:presenter);case .kimi:credential=try await KimiOAuthCoordinator().login(presenting:presenter);default:throw NSError(domain:"QuotaPulse",code:1,userInfo:[NSLocalizedDescriptionKey:"该平台请重新输入 API Key/Token"])};try keychain.saveCredential(credential,accountID:account.id);await SharedStore.shared.clearCooldown(accountID:account.id);if let snapshot=try? await CooldownAwareRefresh.shared.refresh(accountID:account.id,manual:true){snapshots[account.id]=snapshot};WidgetCenter.shared.reloadAllTimelines()}}
