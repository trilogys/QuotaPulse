...
class QuotaPulseWidget : GlanceAppWidget() {
...
class QuotaPulseWidgetReceiver:GlanceAppWidgetReceiver(){override val glanceAppWidget:GlanceAppWidget=QuotaPulseWidget();override fun onDeleted(context:Context,appWidgetIds:IntArray){super.onDeleted(context,appWidgetIds);val store=WidgetConfigStore(context);appWidgetIds.forEach(store::delete)}}
...
private fun summary(context:Context,row:WidgetRow):String{val snapshot=row.snapshot?:return "--";val kind=snapshot.errorKind?:snapshot.errorMessage?.let(ProviderErrorClassifier::classify);if(kind!=null&&snapshot.stale){val retry=row.cooldownUntil?.let{" · ${context.getString(R.string.health_retry_in,formatCountdown(it))}"}.orEmpty();return "${healthText(context,kind)}$retry"};snapshot.balance?.let{return "${it.symbol}${"%.2f".format(it.total)}"};return snapshot.windows.take(2).takeIf{it.isNotEmpty()}?.joinToString(" · "){"${it.label} ${it.remainingPercent.roundToInt()}%"}?:snapshot.connectionLabel?:"--"}
