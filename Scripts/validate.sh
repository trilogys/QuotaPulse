...
assert all(value in http for value in ['SystemVPNDetector', 'NWPathMonitor', 'path.usesInterfaceType(.other)', 'await SystemVPNDetector.isActive()', 'finishTasksAndInvalidate'])
...
assert all(value in proxy_view for value in ['systemVPNActive', 'isCheckingSystemVPN', 'updateSystemVPNStatus', '系统 VPN 已连接', '未检测到系统 VPN', '重新检测 VPN', 'scenePhase', 'await SystemVPNDetector.isActive()'])
...
assert 'versionCode = 10001' in android_gradle and 'versionName = "1.0.1"' in android_gradle
...
grep -q 'MARKETING_VERSION: 1.0.1' project.yml
grep -q 'CURRENT_PROJECT_VERSION: 10001' project.yml
grep -q 'MARKETING_VERSION: 1.0.1' project.single-profile.yml
grep -q 'CURRENT_PROJECT_VERSION: 10001' project.single-profile.yml
